...
function validateRedisConfig(config: RedisClientOptions): void {
  ...
  if (config.database !== undefined && (!Number.isInteger(config.database) || config.database < 0 || config.database > 15)) {
      ...
      reconnectStrategy: (retries: number) => {
        ...
        if (retries <= 0 || retries >= maxRetries) {
          if (retries >= maxRetries) {
            activeLogger.warn({ attempts: retries, maxRetries }, 'Max retry attempts reached. Connection failed.');
          }
          return false; // Stop retrying
        }
        
        // Exponential backoff with jitter: base * 2^(retries-1) + random jitter
        const baseDelay = Math.min(Math.pow(2, retries - 1) * 1000, 16000);
 ...
 */
export function createRedisClient(env: Record<string, string | undefined> = process.env): RedisClientType {
  ...
  if (redisClient) {
    ...
    try {
      ...
      const timeoutPromise = new Promise<never>((_, reject) => 
        setTimeout(() => reject(new Error(`Redis shutdown timeout for ${redisClient?.options?.url}`)), shutdownTimeoutMs)
      );
